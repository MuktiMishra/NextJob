      ...
      <div className="max-w-7xl mx-auto pt-20 px-4 md:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-y-2">
          <p className="text-4xl text-black font-bold">
            Manage Applicants: {job.title}
          </p>
          <p className="text-md text-gray-400">
            Manage and evaluate {job.applications.length || 0} applicants for
            this job
          </p>
        </div>
          ...
          <table className="grow outline outline-1 outline-gray-400 border-collapse rounded-t-xl overflow-hidden">
            <thead className="text-gray-500 text-md bg-gray-200 tracking-tighter border-b border-gray-400">
              <tr>
                ...
                <th className="px-4 py-3 uppercase tracking-wide">Resume</th>
            ...
            <tbody className="text-black text-md bg-white">
              {applicants.length > 0 ? (
                applicants.map((item) => (
                  <tr
                    key={item._id}
                    className="border-b text-center border-gray-200 last:border-0"
                  >
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold border capitalize ${getStatusStyle(item.status)}`}
                      >
                        {item.status || "Pending"}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {item.applicant.profile?.resume ? (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 font-medium underline underline-offset-2"
                        >
                          📄{" "}
